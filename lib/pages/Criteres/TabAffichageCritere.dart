...
Future<Map<String, List<String>>> fetchElements() async {
  ...
  for (var critere in critereData) {
    String critereName = critere['nom_critere'];
    List<String> indicateur = [];

    for (var indicateurItem in indicateurData) {
      if (indicateurItem['id_critere'] == critere['id_critere']) {
        indicateur.add(indicateurItem['intitule_indic']);
      }
    }

    elementsMap[critereName] = indicateur;
  }
  print(elementsMap);
  return elementsMap;
}

class ListersCriteres extends StatefulWidget {
  const ListersCriteres({super.key});

  @override
  State<ListersCriteres> createState() => _ListersCriteresState();
}

class _ListersCriteresState extends State<ListersCriteres> {
            ...
            child: InkWell(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: isOpen ? Colors.transparent  : critereColor,
                  border: Border.all(
                    color: isOpen ? critereColor : Colors.black,
                    width: 3.0,
                  ),
                ),
                child: Padding(
                        ...
                        children: [
                          Text(
                            ...
                            style: TextStyle(
                              ...
                              color: isOpen ? critereColor : Colors.white,
                            ),
                          ), 
